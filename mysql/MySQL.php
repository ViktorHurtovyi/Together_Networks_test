...
$sql = 'SELECT DISTINCT category.name FROM category LEFT JOIN product_category 
ON category.id = product_category.category_id LEFT JOIN product ON product.id = product_category.product_id 
...
//гибриды)
$sql = 'SELECT DISTINCT * FROM category LEFT JOIN product_category ON category.id = product_category.category_id 
LEFT JOIN product ON product.id = product_category.product_id WHERE category.id = 1 OR category.id IN 
...
$sql = 'SELECT category.name, COUNT(*) FROM category LEFT JOIN product_category 
ON category.id = product_category.category_id LEFT JOIN product ON product.id = product_category.product_id 
WHERE category.id IN (1, 2, 3, 5, 6 ,7) GROUP BY category.name';
...
$sql = 'SELECT DISTINCT category.name, COUNT(*) FROM category LEFT JOIN product_category 
ON category.id = product_category.category_id LEFT JOIN product ON product.id = product_category.product_id 
WHERE category.id IN (1, 2, 3, 5, 6 ,7) GROUP BY category.name';
...
$sql = 'SELECT DISTINCT c.name, c2.name, c3.name, c4.name from category as c LEFT JOIN category as c2 
ON c2.id = c.parent_id LEFT JOIN category as c3 ON c3.id = c2.parent_id LEFT JOIN category as c4 
ON c4.id = c3.parent_id WHERE c.id = 10';